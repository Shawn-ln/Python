        ...
        for n in range(1, 200):
            ...
            if res == False:
                ...
                if len(BTMAC) == 12:
                    ...
                    # 抛出异常对象
                    raise ex
                UUID_BTMAC = BTMAC[0:2] + ' ' + BTMAC[2:4] + ' ' + BTMAC[4:6] + ' ' + BTMAC[6:8] + ' ' + BTMAC[
                                                                                                         8:10] + ' ' + BTMAC[
                                                                                                                       10:12]
                UUID_WIFIMAC = WIFIMAC[2:4] + ' ' + WIFIMAC[0:2] + ' ' + WIFIMAC[6:8] + ' ' + WIFIMAC[
                                                                                              4:6] + ' ' + WIFIMAC[
                                                                                                           8:10] + ' ' + WIFIMAC[
                                                                                                                         10:12]
                print(UUID_BTMAC)
                ...
                support.file_info(file_path=r'C:\WinTest\LogFile', act='write', file_name='UUID.BAT',
                ...
                # print(reau)
                writeuuid = support.test(tool_path=dictor['tool_path'], result_log_name='writeuuid.txt', act='find',
                                         check_item='Success', check_data='Success!', instruct=struct)
                print('writeuuid: ', writeuuid)
                support.file_info(file_path='C:\WinTest\LogFile', act='write', file_name='UUIDWRTOK.LOG',
                                  param=StartTime + ' UUID Write OK')
                os.system('shutdown -r -t 3')
                break
            # check UUID
            print('check UUID')
            UUID = support.test(tool_path=dictor['tool_path'], act='read', result_log_name='checkuuid.txt',
                                check_item='UUID',
                                instruct=r'WMIC CSPRODUCT LIST FULL | FIND /I "UUID=" >checkuuid.txt', check_data='')
            print('机器写入UUID: ' + UUID)
