# Coding by LiXiao
# Datatime:4/27/2021 8:47 PM
# Filename:Ext_Audio.py
# Toolby: PyCharm
...
dictor = {
    ...
    'RUNITEM': 'Ext_Audio',
    'Errorcode': 'MBCF4',
    'tool_path': r'C:\WinTest\FFT\External_Audio',
    'instruct': r'ScanLog.exe >External_Mic_result.log',
    'result_log_name': 'External_Mic_result.log',
...
try:
    ...
    for n in range(1, 200):
        ...
        os.system(r'WinAudioControl.exe 31')
        JDSTATUS = True
        while JDSTATUS:
            print('********************    请插入音源线做外部麦克风测试   *********************')
            print('********************    请插入音源线做外部麦克风测试   *********************')
            print('********************    请插入音源线做外部麦克风测试   *********************')
            time.sleep(2)
            support.del_log(
        ...
        if rst:
            ...
            support.copy_log(
                ...
                target_path=r'%s\External_Mic_data.log' % (dictor['tool_path']),
                act='w'
            )
            support.copy_log(
                source_path=r'%s\all.txt' % dictor['tool_path'],
